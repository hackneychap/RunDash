...
def blocks_list(request):
    ...
    # ⚡ Bolt Optimization: Use Subqueries to push aggregations down to the DB
    # This prevents loading thousands of RunActivity and BRace objects into memory just to count/sum them.
    km_sq = RunActivity.objects.filter(training_block=OuterRef('pk')).values('training_block').annotate(total=Sum('distance_km')).values('total')
    runs_sq = RunActivity.objects.filter(training_block=OuterRef('pk')).values('training_block').annotate(total=Count('id')).values('total')
    braces_sq = BRace.objects.filter(training_block=OuterRef('pk')).values('training_block').annotate(total=Count('id')).values('total')

    all_blocks = TrainingBlock.objects.select_related('a_race').annotate(
        total_km_db=Coalesce(Subquery(km_sq), 0.0),
        total_runs_db=Coalesce(Subquery(runs_sq), 0),
        b_races_count_db=Coalesce(Subquery(braces_sq), 0)
    ).order_by('-start_date')
