...
from django.db.models import Sum, Avg, Count, OuterRef, Subquery, FloatField, IntegerField
...
def blocks_list(request):
    ...
    # ⚡ Bolt Optimization: Use DB-level aggregation via Subquery/OuterRef instead of
    # loading all activities into Python memory via prefetch_related for summing.
    # This avoids severe serialization and memory overhead.
    activities_stats = RunActivity.objects.filter(
    ...
    all_blocks = TrainingBlock.objects.select_related('a_race').annotate(
        total_km_db=Coalesce(Subquery(activities_stats.values('total_km')[:1], output_field=FloatField()), 0.0),
        total_runs_db=Coalesce(Subquery(activities_stats.values('total_runs')[:1], output_field=IntegerField()), 0),
        b_races_count_db=Coalesce(Subquery(b_races_count_sq.values('count')[:1], output_field=IntegerField()), 0)
    ).order_by('-start_date')
