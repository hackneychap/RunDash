...
def blocks_list(request):
    ...
    # ⚡ Bolt Optimization: Use Subquery to aggregate activities and B races at the DB level,
    # avoiding memory bloat and cartesian product issues when aggregating multiple relations.
    ...
    all_blocks = TrainingBlock.objects.select_related('a_race').annotate(
        db_total_km=Subquery(activities_stats.values('total_km')[:1]),
        db_total_runs=Subquery(activities_stats.values('total_runs')[:1]),
        db_b_races_count=Subquery(b_races_count.values('count')[:1])
    ).order_by('-start_date')
    ...
    for block in all_blocks:
        ...
        total_km = block.db_total_km or 0
        total_runs = block.db_total_runs or 0
        b_races_count = block.db_b_races_count or 0
