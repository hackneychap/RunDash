...
def blocks_list(request):
    ...
    from django.db.models import Subquery, OuterRef, Sum, Count, FloatField, IntegerField
    from django.db.models.functions import Coalesce
    from .models import BRace

    today = date.today()

    # ⚡ Bolt Optimization: Use Subquery to push aggregates to the DB level
    # instead of doing nested loops over block activities in Python.
    # This prevents loading large amounts of activity objects into memory for counting/summing.
    activities_km = RunActivity.objects.filter(
    ...
    all_blocks = TrainingBlock.objects.select_related('a_race').annotate(
        opt_total_km=Coalesce(Subquery(activities_km, output_field=FloatField()), 0.0),
        opt_total_runs=Coalesce(Subquery(activities_count, output_field=IntegerField()), 0),
        opt_b_races_count=Coalesce(Subquery(b_races_count_sq, output_field=IntegerField()), 0)
    ).order_by('-start_date')
    ...
    for block in all_blocks:
        ...
        blocks_with_stats.append({
            ...
            'total_km': round(block.opt_total_km, 1),
            'total_runs': block.opt_total_runs,
            'b_races_count': block.opt_b_races_count,
        })
