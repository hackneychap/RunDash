...
def blocks_list(request):
    ...
    # ⚡ Bolt Optimization: Use Subqueries to aggregate relation counts & sums in the DB
    # Replaces loading all RunActivity and BRace objects into Python memory via prefetch_related
    # Avoiding .annotate(Sum(...)) directly prevents SQL JOIN fanout/Cartesian products.
    ...
    b_races_count = BRace.objects.filter(
        training_block=OuterRef('pk')
    ).values('training_block').annotate(
        total=Count('id')
    ).values('total')
    ...
    all_blocks = TrainingBlock.objects.select_related('a_race').annotate(
        total_km=Subquery(activities_sum, output_field=FloatField()),
        total_runs=Subquery(activities_count, output_field=IntegerField()),
        total_b_races=Subquery(b_races_count, output_field=IntegerField())
    ).order_by('-start_date')
    ...
    for block in all_blocks:
        ...
        blocks_with_stats.append({
            ...
            'total_km': round(block.total_km or 0, 1),
            'total_runs': block.total_runs or 0,
            'b_races_count': block.total_b_races or 0,
        })
