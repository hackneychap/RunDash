...
def blocks_list(request):
    ...
    from django.db.models import OuterRef, Subquery, Sum, Count, FloatField, IntegerField
    from django.db.models.functions import Coalesce
    from .models import BRace

    today = date.today()

    # ⚡ Bolt Optimization: Replace in-memory python loop calculations with database subqueries.
    # Eliminates fetching every activity into memory just to count/sum them.
    activities_summary = RunActivity.objects.filter(
        training_block=OuterRef('pk')
    ).values('training_block')

    km_subquery = activities_summary.annotate(
        sum_km=Sum('distance_km')
    ).values('sum_km')

    runs_subquery = activities_summary.annotate(
        count_runs=Count('id')
    ).values('count_runs')

    b_races_summary = BRace.objects.filter(
        training_block=OuterRef('pk')
    ).values('training_block').annotate(
        count_b_races=Count('id')
    ).values('count_b_races')

    all_blocks = TrainingBlock.objects.select_related('a_race').annotate(
        total_km_opt=Coalesce(Subquery(km_subquery, output_field=FloatField()), 0.0),
        total_runs_opt=Coalesce(Subquery(runs_subquery, output_field=IntegerField()), 0),
        b_races_count_opt=Coalesce(Subquery(b_races_summary, output_field=IntegerField()), 0)
    ).order_by('-start_date')
    ...
    for block in all_blocks:
        ...
        blocks_with_stats.append({
            ...
            'total_km': round(block.total_km_opt, 1),
            'total_runs': block.total_runs_opt,
            'b_races_count': block.b_races_count_opt,
        })
