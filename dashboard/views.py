...
def blocks_list(request):
    ...
    from django.db.models import Sum, Count, Subquery, OuterRef, FloatField, IntegerField
    from .models import RunActivity, BRace
    today = date.today()

    # ⚡ Bolt Optimization: Use Subquery to push aggregates down to the DB
    # Instead of pulling hundreds of RunActivity objects into Python memory
    # just to compute total distance and count, we do it natively in SQLite.
    # This avoids N+1 memory issues and instantiation overhead.
    activities_agg = RunActivity.objects.filter(
    ...
    braces_agg = BRace.objects.filter(
        training_block=OuterRef('pk')
    ).values('training_block').annotate(
        b_races_count=Count('id')
    ).values('b_races_count')

    all_blocks = TrainingBlock.objects.select_related('a_race').annotate(
        total_km_annotated=Subquery(activities_agg.values('total_km'), output_field=FloatField()),
        total_runs_annotated=Subquery(activities_agg.values('total_runs'), output_field=IntegerField()),
        b_races_count_annotated=Subquery(braces_agg, output_field=IntegerField())
    ).order_by('-start_date')
    ...
    for block in all_blocks:
        ...
        # Extract values from database annotations (with defaults)
        total_km = block.total_km_annotated or 0.0
        total_runs = block.total_runs_annotated or 0
        b_races_count = block.b_races_count_annotated or 0
