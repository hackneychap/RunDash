...
def blocks_list(request):
    ...
    from django.db.models import Sum, Count, Subquery, OuterRef, FloatField, Value, IntegerField
    from django.db.models.functions import Coalesce
    from dashboard.models import RunActivity, BRace

    today = date.today()

    # ⚡ Bolt Optimization: Replace O(N*M) in-memory aggregation loop with database-level aggregation.
    # Uses Subquery and OuterRef to safely sum/count related rows without causing SQL JOIN fanout.
    ...
    b_races_stats = BRace.objects.filter(
        training_block=OuterRef('pk')
    ).values('training_block').annotate(
    ...
    all_blocks = TrainingBlock.objects.select_related('a_race').annotate(
        total_km_annotated=Coalesce(
            Subquery(activities_stats.values('total_km')[:1]),
            Value(0.0, output_field=FloatField()),
            output_field=FloatField()
        ),
        total_runs_annotated=Coalesce(
            Subquery(activities_stats.values('total_runs')[:1]),
            Value(0, output_field=IntegerField()),
            output_field=IntegerField()
        ),
        b_races_count_annotated=Coalesce(
            Subquery(b_races_stats.values('total_races')[:1]),
            Value(0, output_field=IntegerField()),
            output_field=IntegerField()
        )
    ).order_by('-start_date')
