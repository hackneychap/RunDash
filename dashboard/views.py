...
from django.db.models import Sum, Avg, Count, Subquery, OuterRef, IntegerField, FloatField
...
def blocks_list(request):
    ...
    # ⚡ Bolt Optimization: Use Subquery to push aggregations to the DB.
    # Avoids fetching all activities and races into memory just to count/sum them.
    activities_sq = RunActivity.objects.filter(training_block=OuterRef('pk'))
    b_races_sq = BRace.objects.filter(training_block=OuterRef('pk'))

    all_blocks = TrainingBlock.objects.select_related('a_race').annotate(
        total_km=Coalesce(
            Subquery(
                activities_sq.values('training_block').annotate(total=Sum('distance_km')).values('total'),
                output_field=FloatField()
            ), 0.0
        ),
        total_runs=Coalesce(
            Subquery(
                activities_sq.values('training_block').annotate(count=Count('id')).values('count'),
                output_field=IntegerField()
            ), 0
        ),
        b_races_count=Coalesce(
            Subquery(
                b_races_sq.values('training_block').annotate(count=Count('id')).values('count'),
                output_field=IntegerField()
            ), 0
        )
    ).order_by('-start_date')
    ...
    for block in all_blocks:
        ...
        blocks_with_stats.append({
            ...
            'total_km': round(block.total_km, 1),
            'total_runs': block.total_runs,
            'b_races_count': block.b_races_count,
        })
