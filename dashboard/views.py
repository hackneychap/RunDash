...
def blocks_list(request):
    ...
    # ⚡ Bolt Optimization: Use Subquery and OuterRef for block aggregations
    # Replaces prefetch_related('activities', 'b_races') which loads all objects into memory
    # to count and sum them in Python. Pushing aggregations to the database reduces memory
    # overhead and query deserialization time significantly.
    ...
    all_blocks = TrainingBlock.objects.select_related('a_race').annotate(
        annotated_total_km=Coalesce(Subquery(activities_km), 0.0),
        annotated_total_runs=Coalesce(Subquery(activities_count), 0),
        annotated_b_races_count=Coalesce(Subquery(b_races_count_sq), 0)
    ).order_by('-start_date')
