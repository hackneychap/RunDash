...
## 2025-02-18 - [Avoid Cartesian Products with Subqueries in Django]
**Learning:** When trying to annotate a Django QuerySet with sums or counts from multiple different child relationships (e.g., `activities` and `b_races` on a `TrainingBlock`), using standard `.annotate(Sum(...), Count(...))` can cause a SQL JOIN explosion (cartesian product) leading to incorrect aggregation results.
**Action:** When performing complex aggregations over multiple one-to-many relationships in Django, always use `Subquery` and `OuterRef` alongside `Coalesce` to calculate these values in isolated subqueries instead of relying on a single large join.
