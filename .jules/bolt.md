...
## 2025-06-18 - [Django Context Caching]
**Learning:** Evaluating complex aggregations and pickling raw QuerySets on every page load causes significant performance overhead. Caching the fully evaluated context dictionary avoids recomputing these aggregations.
**Action:** Use `django.core.cache.cache` to cache the fully evaluated context dictionary, using a dynamic timeout if the context includes calculations relative to the current date (e.g., 'today').
